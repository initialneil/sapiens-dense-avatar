...
class RandomXYZIUVResizeCompensate(BaseTransform):
    ...
    def transform(self, results: dict) -> dict:
        ...
        ### resize gt_xyziuv and mask same as shape of img
        if 'gt_xyziuv' in results.keys():
            gt_xyziuv = results['gt_xyziuv']

            ## keep_ratio is true from RandomResize in the config
            if keep_ratio:
                gt_xyziuv_resized, scale_factor = mmcv.imrescale(
                    gt_xyziuv,
                    results['scale'],
                    interpolation='nearest',
                    return_scale=True,
                ...
                gt_xyziuv_resized, w_scale, h_scale = mmcv.imresize(
                    gt_xyziuv,
                    results['scale'],
                    interpolation='nearest',
                    return_scale=True,
...
@TRANSFORMS.register_module()
class RandomXYZIUVCrop(BaseTransform):
    ...
    def transform(self, results: dict) -> dict:
        ...
        # crop mask and crop xyziuv
        for key in ['mask', 'gt_xyziuv']:
...
@TRANSFORMS.register_module()
class XYZIUVResize(BaseTransform):
...
@TRANSFORMS.register_module()
class XYZIUVRandomFlip(MMCV_RandomFlip):
...
@TRANSFORMS.register_module()
class GenerateXYZIUVTarget(BaseTransform):
...
@TRANSFORMS.register_module()
class PackXYZIUVInputs(BaseTransform):
