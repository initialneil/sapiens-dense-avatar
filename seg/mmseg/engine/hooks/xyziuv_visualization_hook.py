...
class XYZIUVVisualizationHook(Hook):
    ...
    def vis_xyz_map(self, xyz_map, mask=None, colormap='viridis'):
        xyz_map_vis = (((xyz_map + 1.5) / 3.0) * 255).astype(np.uint8)
        xyz_map_vis[mask == 0] = 0
        return xyz_map_vis
    
    def vis_i_map(self, xyziuv_i_map):
        i_map_vis = xyziuv_i_map.squeeze()
        i_map_vis = (i_map_vis[..., None].repeat(3, axis=-1) * 0.5 * 255).astype(np.uint8)
        return i_map_vis
    
    def vis_uv_map(self, uv_map, mask=None, colormap='viridis'):
        xyz_map_vis = (((uv_map + 1.5) / 3.0) * 255).astype(np.uint8)
        xyz_map_vis[mask == 0] = 0
        xyz_map_vis = np.concatenate([xyz_map_vis, np.zeros_like(xyz_map_vis[..., :1])], axis=-1)
        return xyz_map_vis
    ...
    def after_train_iter(self,
        ...
        for i, (input, data_sample_with_pred, seg_logit) in enumerate(zip(inputs, data_samples_with_pred, seg_logits)):
            ...
            gt_xyziuv = data_sample_with_pred.gt_depth_map.data.numpy() ## 6 x H x W
            gt_xyziuv = gt_xyziuv.transpose((1, 2, 0)) ## H x W x 6

            mask = (gt_xyziuv[..., 3] > 1e-3)
            vis_gt_xyz_map = self.vis_xyz_map(gt_xyziuv[..., :3], mask)
            vis_gt_i_map = self.vis_i_map(gt_xyziuv[..., 3])
            vis_gt_uv_map = self.vis_uv_map(gt_xyziuv[..., 4:6], mask)
